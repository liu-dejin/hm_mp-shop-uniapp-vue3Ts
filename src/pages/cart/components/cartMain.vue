...
const selectedCartListMoney = computed(() =>
  selectedCartList.value.reduce((sum, item) => sum + item.count * item.price, 0).toFixed(2),
)

const gotoPayment = () => {
  console.log(selectedCartListCount.value)
  if (selectedCartListCount.value === 0) {
    uni.showToast({ icon: 'none', title: '请选择商品' })
  }
  uni.showToast({ title: '等待支付' })
}
</script>
...
<template>
  <scroll-view scroll-y class="scroll-view">
    ...
    <template v-if="memberStore.profile">
      ...
      <!-- 吸底工具栏 -->
      <view class="toolbar">
        <text class="all" @tap="onChangeSelectAll" :class="{ checked: isSelectedAll }">全选</text>
